...
**MRVL +32.52% on Jensen "trillion-dollar" endorsement — NVLink Fusion ecosystem elevated; ALAB down to $312 pre-Computex:** Jensen Huang publicly endorsed Marvell at Computex June 2 as "the next trillion-dollar company," sending MRVL from $219.26 to ~$290.79 (+32.52%, biggest single-day gain in MRVL history). While ALAB was not mentioned, the broader endorsement validates the connectivity infrastructure layer that ALAB co-occupies with MRVL inside the NVLink Fusion stack. ALAB itself closed June 2 at ~$312.13 (down -8.5% from May 31 carry of $341.02) — a pre-Computex pullback as retail profit-taking offset institutional pre-event positioning. MRVL down -1.41% on June 3 while ALAB up +3.86%, confirming market views the two as complementary within the NVLink Fusion stack rather than competing for the same price appreciation. Source: CNBC / Yahoo Finance, 2026-06-02. *Thesis bearing: Confirms — the NVLink Fusion ecosystem received its highest-profile institutional validation yet; ALAB's parallel positioning within that ecosystem reinforces the "paid regardless of who wins the XPU" framing.*

---

### 2026-06-03

**Computex 2026 press conference — first public Scorpio X 320-lane live demo; UALink support disclosed; platform now explicitly stack-agnostic:** Astera Labs held its press conference at TaiNEX 1 Room 303 (10–11am Taipei, June 3, 2026). Key product showcase: first public live demonstration of the Scorpio™ X-Series 320 Lane Smart Fabric Switch, ultra-low latency linear optical connectivity, COSMOS-enabled rack-level validation, and production-ready ecosystem partner systems. Critical disclosure: the Intelligent Connectivity Platform now formally integrates **CXL®, Ethernet, NVLink Fusion, PCIe®, and UALink™** alongside the COSMOS software suite. UALink is the AMD/Broadcom/Intel-led open interconnect standard — a direct competitor to NVLink. ALAB being certified across both NVLink Fusion (NVIDIA ecosystem) and UALink (AMD/non-NVIDIA ecosystem) transforms the competitive positioning: ALAB is no longer "NVIDIA connectivity partner" but "stack-agnostic AI fabric" that gets paid regardless of which GPU architecture wins the scale-up layer. ALAB closed +3.86% on June 3 to approximately $324 on Computex momentum. Source: Astera Labs IR / GlobeNewswire / StockTitan, 2026-06-03. *Thesis bearing: Confirms (thesis upgrade) — UALink support is the most important strategic disclosure in the ALAB entity. Prior risk: ALAB's NVLink Fusion partnership creates dependency on NVIDIA remaining the dominant AI factory architecture. New framing: ALAB is certified for both NVLink Fusion and UALink, making it architecturally agnostic — exactly meeting the Saturn mandate of "gets paid regardless of which application wins at the top of the stack."*

**CXL KV Cache design win — second hyperscaler, new (unnamed) customer, chips ship 2027:** During the Computex fireside chat (SVP Thad Omura, Compute Connectivity Group), ALAB disclosed a second custom CXL KV Cache offload design win with a new, unnamed hyperscaler customer — the first confirmed CXL design win beyond Amazon. The custom chip accelerates memory access for AI inference workloads: KV cache is the dominant memory bottleneck in large language model inference (storing attention key-value pairs requires continuous high-bandwidth memory access). At-scale performance validation is underway; chips expected to ship for revenue in 2027. This converts 1 of the 10 disclosed hyperscaler engagements into a confirmed custom silicon commitment, establishing ALAB as a custom CXL chip supplier (not just a retimer/switch vendor) across multiple hyperscalers. Source: Astera Labs Computex 2026 / Quiver Quantitative, 2026-06-03. *Thesis bearing: Confirms (strong) — the CXL KV Cache win for a second hyperscaler is the most direct validation of the 2027+ CXL memory pooling thesis. This is not a retimer upsell; it is a custom silicon co-design engagement — the highest-value design win structure. If 1–2 more of the 10 engagements close by year-end as management guided, ALAB's 2027 CXL revenue could be material.*

**NVLink Fusion custom design win — NVIDIA + hyperscaler collaboration formalized:** Astera Labs issued a separate press release ("Astera Labs Expands Connectivity Portfolio with Custom Solutions") disclosing a NVLink Fusion custom silicon design win in collaboration with NVIDIA and a (unnamed) hyperscaler, targeting hybrid racks where non-NVIDIA accelerators interface with NVIDIA AI factory infrastructure. The custom solution sustains "multiple terabytes per second" of low-latency throughput for heterogeneous rack deployments. This converts the May 28 NVLink Fusion ecosystem partnership (collaboration framework) into a confirmed revenue-bearing design win — a meaningful upgrade in commitment level. Source: Astera Labs IR / StockTitan, 2026-06-03. *Thesis bearing: Confirms — the NVLink Fusion design win alongside the CXL KV Cache win demonstrates that ALAB is converting its 10-engagement pipeline ahead of the year-end target; two custom silicon wins disclosed at Computex, versus management's prior guidance of "a couple" for all of 2026. Revenue in 2027 for both programs.*
